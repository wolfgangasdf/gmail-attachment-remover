...
object GmailStuff {
  ...
  def getToDelete: Task[ListBuffer[ToDelete]] = new javafx.concurrent.Task[ListBuffer[ToDelete]] {
    override def call(): ListBuffer[ToDelete] = {
      ...
      reconnect()
      val inbox = store.getFolder(s"$gmailFolderPrefix/$allmailfolder")
...
//          // re-open connection incl auth after 10 minutes... Gmail seems to drop it from time to time.
          ...
          reconnect()
          allmail = store.getFolder(s"$gmailFolderPrefix/$allmailfolder").asInstanceOf[IMAPFolder]
          trash = store.getFolder(s"$gmailFolderPrefix/$trashfolder").asInstanceOf[IMAPFolder]
